...
public class LaunchProfile
{
    ...
    public List<string> SelectedAccountIds { get; init; } = [];
    public List<string> SelectedServerIds { get; init; } = [];
    ...
    /// </summary>
    public Dictionary<string, ProfileCharacterCommands> CharacterLoginCommands { get; init; } = [];
    ...
    /// </summary>
    /// <remarks>
    /// TODO: Composite-string keys are fragile — renaming an account, server, or character
    /// silently breaks lookups. Consider migrating to a typed key (e.g. record CharacterKey)
    /// or entity IDs once the UX accommodates it. Same applies to <see cref="CharacterLoginCommands"/>.
    /// </remarks>
    public Dictionary<string, string> DefaultCharacters { get; init; } = [];
}
