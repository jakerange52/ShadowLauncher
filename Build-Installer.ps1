...
& wix build $msiPkg $msiPriv -d "AppPublishDir=$publishDir\" -d "CustomActionsDir=$caDir\" -d "ThirdPartyDir=$filterDest\..\\" -d "LicenseFile=$licenseFile" -d "Version=$Version" -ext WixToolset.UI.wixext -ext WixToolset.Util.wixext -arch x86 -out $msiOut
